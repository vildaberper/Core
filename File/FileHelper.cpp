...
namespace filehelper {
	...
	static std::string relativeTo(const std::string& fromPath, const std::string& toPath) {
		if (fromPath[0] != toPath[0]) return toPath;
	...
	static std::vector<std::string> listDrives() {
		...
		for (char i = 'A'; i <= 'Z'; ++i) if ((test & (1 << (i - 'A')))) drives.push_back(std::string(1, i) + ':' + FILE_SEPARATOR);
	...
	static std::vector<std::string> listFiles(const std::string& path) {
		...
		if ((dir = opendir(path.data())) != nullptr) {
			while ((ent = readdir(dir)) != nullptr) {
				...
				if (path[path.length() - 1] == FILE_SEPARATOR) files.push_back(path + ent->d_name);
				else files.push_back(path + FILE_SEPARATOR + ent->d_name);
			}
			closedir(dir);
			delete ent;
		}
	...
	static std::streampos size(const std::string& path) {
		...
		if (isDirectory(path)) {
			for (std::string child : listFiles(path)) size_ += size(child);
			return size_;
		}
	...
	static std::string parent(const std::string& path) {
		...
		if (path.find_first_of(FILE_SEPARATOR) == path.find_last_of(FILE_SEPARATOR)) p += FILE_SEPARATOR;
	...
	static std::string child(const std::string& path, const std::string& child) {
		if (path[path.length() - 1] == FILE_SEPARATOR) return path + child;
		return path + FILE_SEPARATOR + child;
	}

	// Check if file exists.
	static bool exists(const std::string& filename) {
		if (isDirectory(filename)) return true;
	...
	static bool remove(const std::string& path) {
		if (isDirectory(path)) return _rmdir(path.data()) == 0;
	...
	static bool mkdirs(const std::string& path) {
		if (!hasParent(path) || isDirectory(path)) return true;
		return mkdirs(parent(path)) && mkdir(path);
	}

	// Returns a vector<string> with lines read from file.
	static bool readTextFile(const std::string& filename, std::vector<std::string>& content) {
		std::string line;
		std::ifstream ifs;

		ifs.open(filename.data(), std::ios_base::in);
		if (ifs.is_open()) while (std::getline(ifs, line)) content.push_back(line);
		else return false;
	...
	static bool writeTextFile(const std::string& filename, const std::vector<std::string>& content) {
		...
		if (ofs.is_open()) for (std::string line : content) ofs << line << '\n';
		else return false;
